  ...
  */
  function solution(s) {
    ...
    //3. 1번을 비교문자열 길이 단위(1,2,3...문자열 길이) 반큼 반복한다.
    ...
    for(var i=0; i<s.length; i++){
      ...
      for(var j=0; j<s.length/cmpChrLength; j++){
        ...
        var end = cmpChrLength*j + cmpChrLength;
      ...
      for(var k=0; k<tempArr.length; k++){
        ...
        for(var l=0; l<tempArr.length; l++){
          if (continueCompare && tempArr[k] === tempArr[l]) {
            ++duplChrNum;
            duplChr = tempArr[k];
            nextIteratorKIdx = l+1;
            nextIteratorLIdx = nextIteratorKIdx+1;
          } else {
